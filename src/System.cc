...
namespace ORB_SLAM2 {
    ...
    System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
                   const bool bUseViewer, const bool bLocalizeInMap) : mSensor(sensor),
        ...
        //Create the Map
        mpMap = new Map(mpKeyFrameDatabase);
    ...
    cv::Mat System::LocalizeRGBD(const cv::Mat &im, const cv::Mat &depthmap) {
        if (mSensor != RGBD) {
            cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
            exit(-1);
        }
        cv::Mat Tcw = mpTracker->LocalizeImageRGBD(im, depthmap);
        return Tcw;
    }
    ...
    bool System::LoadMap(const string &mapfile, const string &parafile) {
        cout << "Loading camera setting from " << parafile << " ... ";
        if (mpMap->LoadCofficient(parafile))
            cout << " Done! " << endl;
        else {
            cout << " Filed! " << endl;
            return false;
